...
class Strategy:
    ...
    def learn(self, single_history, other_bids = []):
        return
...
class CRecorderStrategy(Strategy):
    ...
    def bid(self, valuation):
        now = len(self.other_bids) - 1
        cnt = 0
        for x in self.other_bids:
            if x > valuation * self.k:
                cnt += 1
        if cnt >= self.n:
            return valuation
        else:
